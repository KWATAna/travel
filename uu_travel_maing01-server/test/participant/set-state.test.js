...
const CMD = "participant/setState";
...
describe("Testing participant create command...", () => {
  test("HDS", async () => {
    ...
    let result = await TestHelper.executePostCommand("participant/setState", { id: helpingVar.id, state: "inactive" });
    expect(result.status).toEqual(200);
    expect(result.data.uuAppErrorMap).toBeDefined();
    expect(result.data.state).toEqual("inactive");
  });
});
