...
describe("Testing participant create command...", () => {
  test("HDS", async () => {
    ...
    let result = await TestHelper.executeGetCommand("participant/get", { id: helpingVar.id });
    expect(result.status).toEqual(200);
    expect(result.data.uuAppErrorMap).toBeDefined();
    expect(result.data.name).toEqual("New participant 3");
  });
});
